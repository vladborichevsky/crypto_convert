<template>
  <div>
    <h2 class="mt-7 uppercase text-deep_dark_blue font-medium">Часто используемые валюты:</h2>
    ...
    <div class="mt-3 mb-3.5 mx-auto sm:flex sm:justify-center sm:items-center lg:mt-5">
      <div class="flex flex-col justify-center items-center sm:flex-row">
        <commonly-used-cur-btn
          ...
          :key="index"
          :class="{
            'bg-deep_dark_blue': activeSelector == index,
            'text-white': activeSelector == index,
          }"
      ...
      <convert-button @click.stop="sendDataFromSelectors()" />
    </div>
  </div>
</template>
...
const commonlyUsedCurrenciesArr = [
  ['btc', 'usdt'],
  ['btc', 'eth'],
  ['eth', 'btc'],
  ['eth', 'usdt'],
